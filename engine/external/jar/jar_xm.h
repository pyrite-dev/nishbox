 ...
 * @see jar_xm_create_context_safe()
 */
int jar_xm_create_context(jar_xm_context_t**, const char* moddata, gf_uint32_t rate);
 ...
 * @returns 2 if memory allocation failed
 */
int jar_xm_create_context_safe(jar_xm_context_t**, const char* moddata, size_t moddata_length, gf_uint32_t rate);
 ...
 * @param numsamples number of samples to generate
 */
void jar_xm_generate_samples_16bit(jar_xm_context_t* ctx, gf_int16_t* output, size_t numsamples);
 ...
 * @param numsamples number of samples to generate
 */
void jar_xm_generate_samples_8bit(jar_xm_context_t* ctx, gf_int8_t* output, size_t numsamples);
 ...
 * indefinitely. */
void jar_xm_set_max_loop_count(jar_xm_context_t*, gf_uint8_t loopcnt);
 ...
 * once, etc. */
gf_uint8_t jar_xm_get_loop_count(jar_xm_context_t*);
 ...
 */
gf_bool_t jar_xm_mute_channel(jar_xm_context_t*, gf_uint16_t, gf_bool_t);
 ...
 */
gf_bool_t jar_xm_mute_instrument(jar_xm_context_t*, gf_uint16_t, gf_bool_t);
...
gf_uint16_t jar_xm_get_number_of_channels(jar_xm_context_t*);

/** Get the module length (in patterns). */
gf_uint16_t jar_xm_get_module_length(jar_xm_context_t*);

/** Get the number of patterns. */
gf_uint16_t jar_xm_get_number_of_patterns(jar_xm_context_t*);
 ...
 */
gf_uint16_t jar_xm_get_number_of_rows(jar_xm_context_t*, gf_uint16_t);

/** Get the number of instruments. */
gf_uint16_t jar_xm_get_number_of_instruments(jar_xm_context_t*);
 ...
 */
void jar_xm_get_playing_speed(jar_xm_context_t*, gf_uint16_t* bpm, gf_uint16_t* tempo);
 ...
 * generated audio)
 */
void jar_xm_get_position(jar_xm_context_t*, gf_uint8_t* pattern_index, gf_uint8_t* pattern, gf_uint8_t* row, gf_uint64_t* samples);
 ...
 */
gf_uint64_t jar_xm_get_latest_trigger_of_sample(jar_xm_context_t*, gf_uint16_t instr, gf_uint16_t sample);
...
struct jar_xm_envelope_s {
	...
	gf_uint8_t		num_points;
	gf_uint8_t		sustain_point;
	gf_uint8_t		loop_start_point;
	gf_uint8_t		loop_end_point;
	gf_bool_t		enabled;
	gf_bool_t		sustain_enabled;
	gf_bool_t		loop_enabled;
};
typedef struct jar_xm_envelope_s jar_xm_envelope_t;

struct jar_xm_sample_s {
	char	  name[SAMPLE_NAME_LENGTH + 1];
	gf_int8_t bits; /* Either 8 or 16 */

	gf_uint32_t	   length;
	gf_uint32_t	   loop_start;
	gf_uint32_t	   loop_length;
	gf_uint32_t	   loop_end;
	float		   volume;
	gf_int8_t	   finetune;
	jar_xm_loop_type_t loop_type;
	float		   panning;
	gf_int8_t	   relative_note;
	gf_uint64_t	   latest_trigger;
...
struct jar_xm_instrument_s {
	...
	gf_uint8_t	       sample_of_notes[NUM_NOTES];
	jar_xm_envelope_t      volume_envelope;
	jar_xm_envelope_t      panning_envelope;
	jar_xm_waveform_type_t vibrato_type;
	gf_uint8_t	       vibrato_sweep;
	gf_uint8_t	       vibrato_depth;
	gf_uint8_t	       vibrato_rate;
	gf_uint16_t	       volume_fadeout;
	gf_uint64_t	       latest_trigger;
	gf_bool_t	       muted;
...
struct jar_xm_pattern_slot_s {
	gf_uint8_t note;       /* 1-96, 97 = Key Off note */
	gf_uint8_t instrument; /* 1-128 */
	gf_uint8_t volume_column;
	gf_uint8_t effect_type;
	gf_uint8_t effect_param;
};
...
struct jar_xm_channel_context_s {
	...
	float	  frequency;
	float	  step;
	gf_bool_t ping; /* For ping-pong samples: gf_true is -->, gf_false is <-- */

	float volume;  /* Ideally between 0 (muted) and 1 (loudest) */
	float panning; /* Between 0 (left) and 1 (right); 0.5 is centered */

	gf_uint16_t autovibrato_ticks;

	gf_bool_t   sustained;
	float	    fadeout_volume;
	float	    volume_envelope_volume;
	float	    panning_envelope_panning;
	gf_uint16_t volume_envelope_frame_count;
	gf_uint16_t panning_envelope_frame_count;

	float autovibrato_note_offset;

	gf_bool_t	       arp_in_progress;
	gf_uint8_t	       arp_note_offset;
	gf_uint8_t	       volume_slide_param;
	gf_uint8_t	       fine_volume_slide_param;
	gf_uint8_t	       global_volume_slide_param;
	gf_uint8_t	       panning_slide_param;
	gf_uint8_t	       portamento_up_param;
	gf_uint8_t	       portamento_down_param;
	gf_uint8_t	       fine_portamento_up_param;
	gf_uint8_t	       fine_portamento_down_param;
	gf_uint8_t	       extra_fine_portamento_up_param;
	gf_uint8_t	       extra_fine_portamento_down_param;
	gf_uint8_t	       tone_portamento_param;
	float		       tone_portamento_target_period;
	gf_uint8_t	       multi_retrig_param;
	gf_uint8_t	       note_delay_param;
	gf_uint8_t	       pattern_loop_origin; /* Where to restart a E6y loop */
	gf_uint8_t	       pattern_loop_count;  /* How many loop passes have been done */
	gf_bool_t	       vibrato_in_progress;
	jar_xm_waveform_type_t vibrato_waveform;
	gf_bool_t	       vibrato_waveform_retrigger; /* True if a new note retriggers the waveform */
	gf_uint8_t	       vibrato_param;
	gf_uint16_t	       vibrato_ticks; /* Position in the waveform */
	float		       vibrato_note_offset;
	jar_xm_waveform_type_t tremolo_waveform;
	gf_bool_t	       tremolo_waveform_retrigger;
	gf_uint8_t	       tremolo_param;
	gf_uint8_t	       tremolo_ticks;
	float		       tremolo_volume;
	gf_uint8_t	       tremor_param;
	gf_bool_t	       tremor_on;
...
struct jar_xm_context_s {
	...
	gf_uint8_t  current_table_index;
	gf_uint8_t  current_row;
	gf_uint16_t current_tick; /* Can go below 255, with high tempo and a pattern delay */
	float	    remaining_samples_in_tick;
	gf_uint64_t generated_samples;

	gf_bool_t  position_jump;
	gf_bool_t  pattern_break;
	gf_uint8_t jump_dest;
	gf_uint8_t jump_row;

	/* Extra ticks to be played before going to the next row -
	 * Used for EEy effect */
	gf_uint16_t extra_ticks;

	gf_uint8_t* row_loop_count; /* Array of size MAX_NUM_ROWS * module_length */
	gf_uint8_t  loop_count;
	gf_uint8_t  max_loop_count;
...
int jar_xm_create_context(jar_xm_context_t** ctxp, const char* moddata, gf_uint32_t rate) { return jar_xm_create_context_safe(ctxp, moddata, SIZE_MAX, rate); }

int jar_xm_create_context_safe(jar_xm_context_t** ctxp, const char* moddata, size_t moddata_length, gf_uint32_t rate) {
#if JAR_XM_DEFENSIVE
	int ret;
#endif
	size_t		  bytes_needed;
	...
	/* Initialize most of the fields to 0, 0.f, NULL or gf_false depending on type */
	...
	for(i = 0; i < ctx->module.num_channels; ++i) {
		...
		ch->vibrato_waveform_retrigger = gf_true;
		ch->tremolo_waveform	       = jar_xm_SINE_WAVEFORM;
		ch->tremolo_waveform_retrigger = gf_true;
	...
	ctx->row_loop_count = (gf_uint8_t*)mempool;
	mempool += MAX_NUM_ROWS * sizeof(gf_uint8_t);
...
gf_bool_t jar_xm_mute_instrument(jar_xm_context_t* ctx, gf_uint16_t instr, gf_bool_t mute) {
	gf_bool_t old				 = ctx->module.instruments[instr - 1].muted;
...
gf_uint16_t jar_xm_get_number_of_channels(jar_xm_context_t* ctx) { return ctx->module.num_channels; }

gf_uint16_t jar_xm_get_module_length(jar_xm_context_t* ctx) { return ctx->module.length; }

gf_uint16_t jar_xm_get_number_of_patterns(jar_xm_context_t* ctx) { return ctx->module.num_patterns; }

gf_uint16_t jar_xm_get_number_of_rows(jar_xm_context_t* ctx, gf_uint16_t pattern) { return ctx->module.patterns[pattern].num_rows; }

gf_uint16_t jar_xm_get_number_of_instruments(jar_xm_context_t* ctx) { return ctx->module.num_instruments; }

gf_uint16_t jar_xm_get_number_of_samples(jar_xm_context_t* ctx, gf_uint16_t instrument) { return ctx->module.instruments[instrument - 1].num_samples; }

void jar_xm_get_playing_speed(jar_xm_context_t* ctx, gf_uint16_t* bpm, gf_uint16_t* tempo) {
	if(bpm) *bpm = ctx->bpm;
	if(tempo) *tempo = ctx->tempo;
}

void jar_xm_get_position(jar_xm_context_t* ctx, gf_uint8_t* pattern_index, gf_uint8_t* pattern, gf_uint8_t* row, gf_uint64_t* samples) {
...
gf_uint64_t jar_xm_get_latest_trigger_of_instrument(jar_xm_context_t* ctx, gf_uint16_t instr) { return ctx->module.instruments[instr - 1].latest_trigger; }

gf_uint64_t jar_xm_get_latest_trigger_of_sample(jar_xm_context_t* ctx, gf_uint16_t instr, gf_uint16_t sample) { return ctx->module.instruments[instr - 1].samples[sample].latest_trigger; }

gf_uint64_t jar_xm_get_latest_trigger_of_channel(jar_xm_context_t* ctx, gf_uint16_t chn) { return ctx->channels[chn - 1].latest_trigger; }
 ...
 */
#define READ_U8(offset) (((offset) < moddata_length) ? (*(gf_uint8_t*)(moddata + (offset))) : 0)
#define READ_U16(offset) ((gf_uint16_t)READ_U8(offset) | ((gf_uint16_t)READ_U8((offset) + 1) << 8))
...
size_t jar_xm_get_memory_needed_for_context(const char* moddata, size_t moddata_length) {
	...
	memory_needed += MAX_NUM_ROWS * READ_U16(offset + 4) * sizeof(gf_uint8_t); /* Module length */
...
char* jar_xm_load_module(jar_xm_context_t* ctx, const char* moddata, size_t moddata_length, char* mempool) {
	...
	for(i = 0; i < ctx->module.num_instruments; ++i) {
		...
		for(j = 0; j < instr->num_samples; ++j) {
			...
			sample->relative_note = (gf_int8_t)READ_U8(offset + 16);
...
static float jar_xm_waveform(jar_xm_waveform_type_t, gf_uint8_t);
static void  jar_xm_autovibrato(jar_xm_context_t*, jar_xm_channel_context_t*);
static void  jar_xm_vibrato(jar_xm_context_t*, jar_xm_channel_context_t*, gf_uint8_t, gf_uint16_t);
static void  jar_xm_tremolo(jar_xm_context_t*, jar_xm_channel_context_t*, gf_uint8_t, gf_uint16_t);
static void  jar_xm_arpeggio(jar_xm_context_t*, jar_xm_channel_context_t*, gf_uint8_t, gf_uint16_t);
...
static void  jar_xm_panning_slide(jar_xm_channel_context_t*, gf_uint8_t);
static void  jar_xm_volume_slide(jar_xm_channel_context_t*, gf_uint8_t);

static float jar_xm_envelope_lerp(jar_xm_envelope_point_t*, jar_xm_envelope_point_t*, gf_uint16_t);
static void  jar_xm_envelope_tick(jar_xm_channel_context_t*, jar_xm_envelope_t*, gf_uint16_t*, float*);
...
static const gf_uint16_t amiga_frequencies[] = {
...
static float jar_xm_waveform(jar_xm_waveform_type_t waveform, gf_uint8_t step) {
...
static void jar_xm_vibrato(jar_xm_context_t* ctx, jar_xm_channel_context_t* ch, gf_uint8_t param, gf_uint16_t pos) {
...
static void jar_xm_tremolo(jar_xm_context_t* ctx, jar_xm_channel_context_t* ch, gf_uint8_t param, gf_uint16_t pos) {
	unsigned int step = pos * (param >> 4);
...
static void jar_xm_arpeggio(jar_xm_context_t* ctx, jar_xm_channel_context_t* ch, gf_uint8_t param, gf_uint16_t tick) {
	switch(tick % 3) {
	case 0:
		ch->arp_in_progress = gf_false;
		ch->arp_note_offset = 0;
		break;
	case 2:
		ch->arp_in_progress = gf_true;
...
static void jar_xm_panning_slide(jar_xm_channel_context_t* ch, gf_uint8_t rawval) {
	float f;
...
static void jar_xm_volume_slide(jar_xm_channel_context_t* ch, gf_uint8_t rawval) {
	float f;
...
static float jar_xm_amiga_period(float note) {
	...
	gf_uint8_t   a	     = intnote % 12;
	gf_int8_t    octave  = note / 12.f - 2;
	gf_uint16_t  p1 = amiga_frequencies[a], p2 = amiga_frequencies[a + 1];
...
static void jar_xm_handle_note_and_instrument(jar_xm_context_t* ctx, jar_xm_channel_context_t* ch, jar_xm_pattern_slot_t* s) {
	...
	case 9: /* 9xx: Sample offset */
		if(ch->sample != NULL && NOTE_IS_VALID(s->note)) {
			gf_uint32_t final_offset = s->effect_param << (ch->sample->bits == 16 ? 7 : 8);
...
static void jar_xm_row(jar_xm_context_t* ctx) {
	...
	if(ctx->position_jump) {
		...
		ctx->position_jump	 = gf_false;
		ctx->pattern_break	 = gf_false;
		ctx->jump_row		 = 0;
...
static void jar_xm_envelope_tick(jar_xm_channel_context_t* ch, jar_xm_envelope_t* env, gf_uint16_t* counter, float* outval) {
	if(env->num_points < 2) {
		...
		if(env->loop_enabled) {
			gf_uint16_t loop_start	= env->points[env->loop_start_point].frame;
			gf_uint16_t loop_end	= env->points[env->loop_end_point].frame;
			gf_uint16_t loop_length = loop_end - loop_start;
...
static void jar_xm_tick(jar_xm_context_t* ctx) {
	...
	for(i = 0; i < ctx->module.num_channels; ++i) {
		...
		if(ch->vibrato_in_progress && !HAS_VIBRATO(ch->current)) {
			ch->vibrato_in_progress = gf_false;
			ch->vibrato_note_offset = 0.f;
			jar_xm_update_frequency(ctx, ch);
		}
		...
		case 4: /* 4xy: Vibrato */
			if(ctx->current_tick == 0) break;
			ch->vibrato_in_progress = gf_true;
			jar_xm_vibrato(ctx, ch, ch->vibrato_param, ch->vibrato_ticks++);
			break;
