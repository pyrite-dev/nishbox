...
if _ACTION and _ACTION ~= "clean" then
	...
	f:write("#!/bin/sh\n")
	f:write("set -e\n")
	f:write("mkdir -p $DESTDIR/" .. _OPTIONS["prefix"] .. "/lib\n")
	f:write("mkdir -p $DESTDIR/" .. _OPTIONS["prefix"] .. "/bin\n")
	f:write("mkdir -p $DESTDIR/" .. _OPTIONS["prefix"] .. "/share/NishBox\n")
	f:write("cp ./engine/lib/*/*/*.so $DESTDIR/" .. _OPTIONS["prefix"] .. "/lib/ >/dev/null 2>&1 || echo 'Failed to copy engine, but it is okay'\n")
	f:write("cp ./bin/*/*/nishbox $DESTDIR/" .. _OPTIONS["prefix"] .. "/bin/\n")
	f:write("cp ./base.pak $DESTDIR/" .. _OPTIONS["prefix"] .. "/share/NishBox/\n")
